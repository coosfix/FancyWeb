...
namespace FancyWeb.Areas.Backend.Models
{
    public class CharHub : Hub
    {
        ...
        public void searchuser(int id, string str)
        {
            ...
            foreach (var x in q)
            {
                Clients.Client(Context.ConnectionId).userlist(x.UserID, x.UserName);
            }
        }
        ...
        public void login(int id, string name)
        {
            ...
            //讀出login使用者的全部有過對話的使用者訊息 (上方) Note-04
            ...
            {
                var user = db.Users.Where(x => x.UserID == a.Key).Select(x => new { x.UserID, x.UserName }).FirstOrDefault();
            ...
            {
                Clients.Client(Context.ConnectionId)._messageuser(a.id, a.name, a.talkcontent, a.isreadCnt);
            }
        ...
        public void sendMsg(int fromid, int toid, string message)
        {
            ...
            //在呼叫端顯示
            Clients.Client(Context.ConnectionId).sendMessage(fromid, onlineuser[fromid].name, toid, message, msg.CreateDate.ToString("yyyy年MM月dd日 H:mm"), msgid);

            //被叫端若有上線則顯示訊息
            if (onlineuser.ContainsKey(toid) && fromid != toid)
            {
                Clients.Client(onlineuser[toid].connectid).sendMessage(fromid, onlineuser[fromid].name, toid, message, msg.CreateDate.ToString("yyyy年MM月dd日 H:mm"), msgid);
            }
        }
        ...
        public void callmessage(int fromid, int toid)
        {
            if (toid > 0)
            {
                ...
                {
                    string date = "";
                    if (a.FromUserID == fromid && a.IsRead == true)
                    { date = a.CreateDate.ToString("yyyy年MM月dd日 H:mm") + " (已讀) "; }
                    else
                    { date = a.CreateDate.ToString("yyyy年MM月dd日 H:mm "); }

                    //訊息內容   Note-08
                    Clients.Client(Context.ConnectionId).messagelog(a.FromUserID, a.FromUserName, a.TalkContent, date, a.MessageID);
                }
            }
        }
